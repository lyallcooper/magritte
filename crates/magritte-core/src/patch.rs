...
impl Repo {
    /// Apply a patch/diff file to the working tree (`git apply <path>`), without
    /// creating a commit. Fails (surfaced) if it doesn't apply cleanly.
    pub fn apply_patch_file(&self, path: &str) -> Result<String> {
        let out = self.run(["apply", path])?;
        Ok(out.status_line())
    }

    /// Apply a mailbox of patches as commits (`git am <path>`). A conflict leaves
    /// the am paused, which the sequence banner surfaces (continue/skip/abort).
    ...
    pub fn format_patch(&self, args: &str) -> Result<String> {
