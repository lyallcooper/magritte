...
| `app_icon` | `pipe` / `golconda` / `son-of-man` | `son-of-man` | macOS only. The Dock (and Cmd-Tab) icon. Sets the running app's icon, not the Finder icon, which macOS keeps fixed to the bundle default. |
