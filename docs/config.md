...
| `commit_vim_mode` | `true` / `false` | `false` | Modal Vim editing in the in-app commit editor: Normal/Insert/Visual modes, motions, counts, text objects, `d`/`c`/`y` operators, surround (`ys`/`cs`/`ds`), `.` repeat, `/` search (smartcase, with live match highlighting), and `u`/`Ctrl-r` undo. Commit with `ZZ` or `,,`, cancel with `ZQ` or `,k`; `gq` is the reflow operator (`gqq` line, Visual `gq` selection, `gq{motion}`, `,q` whole message). For full Vim fidelity use `commit_in_editor` with `commit_editor = "nvim"` instead. |
